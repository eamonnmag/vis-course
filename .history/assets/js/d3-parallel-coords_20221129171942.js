...
var parallel_coordinates = (function () {
    ...
    var brush = function () {
        ...
        for (var fileIn in foreground) {
            dimensions.forEach(function (p) {
                console.log(y[fileIn][p].brush.extent());
    ...
    return {
        ...
        loadAndDraw: function (placement, files, width, height, ordinalColumns, options) {
            ...
            x = d3.scalePoint().range([0, width], 1);

            for (var fileIndex in files) {

                d3.csv(files[fileIndex]).then(function (data) {
                    ...
                    x.domain(dimensions = Object.keys(data[0]).filter(function (d) {
                        ...
                        if (ordinalColumns.indexOf(d) != -1) {
                            ...
                            y[data_index][d] = d3.scalePoint().domain(data.map(function (p) {
                                return p[d];
                            })).range([graphHeight, 0]);
                        } else {
                            y[data_index][d] = d3.scaleLinear()
                    ...
                    var g = svg.selectAll(".dimension")
                        ...
                        .call(d3.drag()
                            ...
                            .on("drag", function (e, d) {
                                dragging[d] = Math.min(width, Math.max(0, this.__origin__ += e.dx));
                    ...
                    g.append("g")
                        ...
                        .each(function (d, i) {
                            d3.selectAll("g.brush-" + fileIndex + "-" + i).call(y[data_index][d].brush = d3.brushY(y[data_index][d]).on("brush", brush));
                        })
                        .selectAll("rect")
                        .attr("x", -8)
                        .attr("width", 16);
